...
package io.sarl.extensions.bspl.lang.compiler.fragments;
...
import io.sarl.extensions.bspl.lang.compiler.IProtocolNames;
import io.sarl.extensions.bspl.lang.compiler.generic.ISarlTargetGeneratorContext;
...
public class BsplProtocolSkillGeneratorFragment {
		...
		for (final var messageMapping : sendableMessages.entrySet()) {
			...
			for (final var messageInstance : messageMapping.getValue()) {
				...
				if (!guardParams.isEmpty()) {
					...
					var first = true;
					for (final var guardParam : guardParams) {
						if (first) {
							first = false;
						} else {
				...
				final var inoptParams = messageInstance.getArguments().stream().filter(it -> it.isInput() || it.isOptional()).collect(Collectors.toList());
				if (!inoptParams.isEmpty()) {
					var keyIndex = 0;
					for (final var inoptParam : inoptParams) {
						...
						if (inoptParam.isKey()) {
							...
							receiver.newLine()
								.append("val ").append(varInParamId).append(" = new ").append(names.getKnowledgeNameGenericInterface()) //$NON-NLS-1$ //$NON-NLS-2$
	...
	private static void generateSendFunction(ITreeAppendable receiver, ISarlTargetGeneratorContext<IProtocolNames> context, BsplProtocolRole role, String messageName, Map<String, List<BsplProtocolMessage>> sendableMessages,
		...
		if (!outParams.isEmpty()) {
			...
			for (final var outParam : outParams) {
				...
				receiver.newLine().append("val ").append(idName).append(" = new ").append(names.getKnowledgeNameGenericInterface()).append("("); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				if (!inKeys.isEmpty()) {
	...
	private static void generateProtocolSkill(List<BsplProtocolMessage> messages, BsplProtocolRole role, List<BsplProtocolParameter> parameters,
		...
		if (!sentMessages.isEmpty()) {
			...
			.newLine().append("uses ").append(names.getAgentWorkingMemoryGenericInterface()).append(", ").append(ExternalContextAccess.class); //$NON-NLS-1$ //$NON-NLS-2$
