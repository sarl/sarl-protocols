...
package io.sarl.extensions.bspl.lang.validation;

import static io.sarl.extensions.bspl.lang.validation.IssueCodes.*;
...
import io.sarl.extensions.bspl.lang.bspl.BsplProtocolArgument;
...
public class BSPLValidator extends AbstractBSPLValidator {
	...
	public void checkParameters(BsplProtocol protocol) {
		...
		final var declaredPrivateParameters = new TreeMap<String, BsplProtocolParameter>();
			...
			if (member instanceof BsplProtocolParameter parameter) {
				...
				if (parameter.isOutput()) {
					...
					if (parameter.isPrivateVisibility()) {
						declaredPrivateParameters.put(parameter.getName(), parameter);
						if (parameter.isAny()) {
							// If the parameter is private and explicitly marked with "any", then it is assumed that only "in" is valid. "out" is ignored
							if (!isIgnored(INVALID_PARAMETER_MODIFIER) && parameter.getModifiers().contains("any")) { //$NON-NLS-1$
								addIssue(
										MessageFormat.format(Messages.SARL_BSPLValidator_25, "any", "in", parameter.getName(), protocol.getName()), //$NON-NLS-1$ //$NON-NLS-2$
										parameter,
										INVALID_PARAMETER_MODIFIER);
							}
						} else {
							error(
									MessageFormat.format(Messages.SARL_BSPLValidator_30, parameter.getName(), protocol.getName()),
									parameter,
									BsplPackage.Literals.BSPL_PROTOCOL_PARAMETER__NAME,
									INVALID_PARAMETER_MODIFIER);
						}
				...
				for (final var argument : message.getArguments()) {
					...
					if (argument.isOutput()) {
						declaredPrivateParameters.remove(n);
					}
					++i;
				}
			}
		...
		if (!declaredPrivateParameters.isEmpty()) {
			for (final var parameter : declaredPrivateParameters.values()) {
				error(
						MessageFormat.format(Messages.SARL_BSPLValidator_31, parameter.getName(), protocol.getName()),
						parameter,
						BsplPackage.Literals.BSPL_PROTOCOL_PARAMETER__NAME,
	...
	/** Check the modifiers of the arguments.
	 *
	 * @param argument the SARL BSPL argument.
	 */
	@Check(CheckType.FAST)
	public void checkArgumentModifiers(BsplProtocolArgument argument) {
		if (argument.getModifiers().size() > 1) {
			final var declaredModifier = new TreeSet<String>();
			var i = 0;
			for (final var modifier : argument.getModifiers()) {
				if (!"key".equals(modifier)) { //$NON-NLS-1$
					if (!declaredModifier.add(modifier)) {
						if (!isIgnored(UNNECESSARY_ARGUMENT_MODIFIER)) {
							addIssue(MessageFormat.format(Messages.SARL_BSPLValidator_28, modifier, argument.getName()),
									argument,
									BsplPackage.Literals.BSPL_PROTOCOL_ARGUMENT__MODIFIERS,
									i,
									UNNECESSARY_ARGUMENT_MODIFIER);
						}
					} else if (i > 0 && declaredModifier.size() > 1) {
						error(
								MessageFormat.format(Messages.SARL_BSPLValidator_29, modifier, argument.getName()),
								argument,
								BsplPackage.Literals.BSPL_PROTOCOL_ARGUMENT__MODIFIERS,
								i,
								INVALID_ARGUMENT_MODIFIER);
