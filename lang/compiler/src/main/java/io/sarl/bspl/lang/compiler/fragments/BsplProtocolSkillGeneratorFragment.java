			...
			for (final var messageInstance : messageMapping.getValue()) {
				...
				if (!guardParams.isEmpty()) {
					...
					for (final var guardParam : guardParams) {
						...
						.append("\", ").append(varScope).append(".keys)."); //$NON-NLS-1$ //$NON-NLS-2$
					...
					for (final var inoptParam : inoptParams) {
						...
						if (parameter == null || !parameter.isPrivateVisibility()) {
							if (inoptParam.isKey()) {
								...
								receiver.newLine()
									.append("val ").append(varInParamId).append(" = new ").append(names.getKnowledgeIdGenericInterface()).append("(\"").append(inParamName) //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
									.append("\", ").append(varScope).append(".keys)"); //$NON-NLS-1$ //$NON-NLS-2$
